  ...
  <div class="space-y-6">
    <section v-if="canReceiveInventory" class="bg-white rounded-lg border border-outline-variant/10">
      <div class="p-5 border-b border-surface-container-low flex items-center justify-between">
        ...
        <button class="text-xs text-primary font-semibold" @click="loadPendingProductionOrders">刷新</button>
      </div>
      <div class="p-5">
        <div v-if="pendingProductionOrders.length === 0" class="text-sm text-on-surface-variant">暂无待确认的生产入库订单。</div>
        <table v-else class="w-full text-sm">
          ...
          <tbody>
            <tr v-for="order in pendingProductionOrders" :key="order.id" class="border-t border-outline-variant/20">
              <td class="py-3 font-semibold">{{ order.orderNo }}</td>
              <td class="py-3">{{ formatProducts(order.items) }}</td>
              <td class="py-3">{{ formatItemsQuantity(order.items) }}</td>
    ...
    <section v-if="canReceiveInventory" class="bg-white rounded-lg border border-outline-variant/10">
      <div class="p-5 border-b border-surface-container-low flex items-center justify-between">
        <div>
          <h3 class="text-sm font-bold tracking-tight">待确认采购入库</h3>
          <p class="mt-1 text-xs text-on-surface-variant">采购管理员通知仓库后，仓库管理员确认即可自动完成原材料入库。</p>
        </div>
        <button class="text-xs text-primary font-semibold" @click="loadPendingPurchaseOrders">刷新</button>
      </div>
      <div class="p-5">
        <div v-if="pendingPurchaseOrders.length === 0" class="text-sm text-on-surface-variant">暂无待确认的采购入库单。</div>
        <table v-else class="w-full text-sm">
          <thead class="text-xs text-on-surface-variant">
            <tr class="text-left">
          ...
          <tbody>
            <tr v-for="order in pendingPurchaseOrders" :key="order.id" class="border-t border-outline-variant/20">
              <td class="py-3 font-semibold">{{ order.poNo }}</td>
              <td class="py-3">{{ order.supplier?.name || '-' }}</td>
              <td class="py-3">{{ summarizePurchaseItems(order.items) }}</td>
              <td class="py-3">¥{{ formatQuantity(order.totalAmount) }}</td>
              <td class="py-3">
                <button class="text-xs text-primary font-semibold" @click="confirmPurchaseReceipt(order.id)">确认采购入库</button>
              </td>
            </tr>
          </tbody>
        </table>
...
const canReceiveInventory = computed(() => auth.hasPermission('inventory:receive'));
...
const loadPendingProductionOrders = async () => {
  if (!canReceiveInventory.value) {
    pendingProductionOrders.value = [];
    return;
  }
  try {
    const response = await orderApi.listPendingProductionStockIn();
    pendingProductionOrders.value = response.data || [];
  } catch (err) {
    pendingProductionOrders.value = [];
  }
};
...
const loadPendingPurchaseOrders = async () => {
  ...
  try {
    const response = await procurementApi.listPendingWarehouseReceipts();
...
const confirmProductionStockIn = async (orderId) => {
  ...
  error.value = '';
  try {
    await orderApi.warehouseStockIn(orderId, {});
    message.value = `订单 ${orderId} 生产入库确认成功。`;
...
const confirmPurchaseReceipt = async (orderId) => {
  message.value = '';
  error.value = '';
  try {
