  ...
  setDashboardState({
    ...
    entries: sortByLatest([
      ...requests.map((request) => ({
        ...
        subDetail: `来源：${request.salesOrder?.orderNo || request.productionPlan?.planNo || '-'} · 成品：${request.finishedProduct?.name || '-'} · ${request.warehouseNote || request.requestNote || '等待仓库处理'}`,
  ...
  setDashboardState({
    ...
    entries: sortByLatest([
      ...materialRequests.map((request) => ({
        ...
        title: request.requestNo || `申请#${request.id}`,
        detail: summarizeMaterialItems(request.items),
        subDetail: `来源：${request.salesOrder?.orderNo || request.productionPlan?.planNo || '-'} · 申请人：${request.createdByName || '-'} · ${request.requestNote || '待仓库审核'}`,
        status: request.status || '-',
