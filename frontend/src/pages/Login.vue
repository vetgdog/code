<template>
  <div class="min-h-screen flex items-center justify-center bg-slate-100 px-6 py-10">
    <div class="w-full max-w-5xl grid grid-cols-1 lg:grid-cols-2 rounded-2xl overflow-hidden border border-slate-200 shadow-[0_12px_30px_rgba(15,23,42,0.08)] bg-white">
      <section class="hidden lg:flex flex-col justify-between p-10 bg-slate-900 text-white">
        <div>
          <p class="text-xs uppercase tracking-[0.24em] text-slate-300">Smart Manufacturing Console</p>
          <h2 class="mt-4 text-3xl font-black tracking-tight">SteelOps Precision</h2>
          <p class="mt-4 text-sm text-slate-200 leading-6">连接订单、采购、生产与质量追溯，统一企业协同工作台。</p>
        </div>
      ...
      <section class="p-8 md:p-10">
        ...
        <form class="mt-6 space-y-4" @submit.prevent="handleLogin">
          ...
          <button class="w-full rounded-lg bg-blue-600 hover:bg-blue-700 text-white py-2.5 text-sm font-semibold transition-colors">登录系统</button>
        </form>
