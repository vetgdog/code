    ...
    <section class="bg-white rounded-lg border border-outline-variant/10">
      <div class="p-5 border-b border-surface-container-low flex items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
          ...
          <button v-if="canGenerateProductionPlan" class="rounded bg-primary text-white px-3 py-2 text-sm font-semibold" @click="regeneratePlan">重新生成</button>
        </div>
      </div>
      <div class="p-5">
        <div v-if="!canGenerateProductionPlan" class="mb-3 text-xs text-on-surface-variant">系统管理员在此页面仅支持查看当前与历史生产计划，不可重新生成计划。</div>
...
const canGenerateProductionPlan = computed(() => auth.hasPermission('production:plan:generate'));
...
const regeneratePlan = async () => {
  if (!canGenerateProductionPlan.value) {
    error.value = '当前角色仅支持查看生产计划，不能重新生成计划。';
    return;
  }
  message.value = '';
  error.value = '';
